...
function isString(value) {
  ...
  if (typeof value === 'object') return String(value) !== '[object Object]';
  return false;
}
...
function removeLastOccurrences(str, value) {
  const startLast = str.lastIndexOf(value);
  if (startLast < 0) return str;
  return str.slice(0, startLast) + str.slice(startLast + value.length);
}
...
function countVowels(str) {
  const volwes = ['a', 'e', 'i', 'o', 'u', 'y', 'A', 'E', 'I', 'O', 'U', 'Y'];
  let result = 0;
  str.split('').forEach((char) => {
    if (volwes.includes(char)) result += 1;
  }, 0);
  return result;
}
...
function isPalindrome(str) {
  const upperString = str.toLocaleUpperCase().replaceAll(/[ ?!,]/g, '');
  const reverseStr = upperString.split('').reverse().join('');
  return upperString === reverseStr;
}
...
function encodeToRot13(str) {
  const arrChars = str.split('').map((char) => {
    if (char.charCodeAt() < 65) return char;
